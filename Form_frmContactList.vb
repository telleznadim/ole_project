...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_frmContactList"
...
Private Sub btnClearSelected_Click()
    Set dbs = CurrentDb
    SqlQuery = "UPDATE tblContacts SET tblContacts.IsSelected = 0 WHERE (((tblContacts.IsSelected)=True))"
...
Private Sub btnFilter_Click()
    ...
    If FilterGood = True Then
        Select Case FilterBy
            Case "Name"
                SqlQuery = "([tblContacts].[FullName] Like '*" & FilterFor & "*') AND "
            Case "Company"
                SqlQuery = "([tblContacts].[Company] Like '*" & FilterFor & "*') AND "
            Case "Department"
                SqlQuery = "([tblContacts].[Department] Like '*" & FilterFor & "*') AND "
        End Select
...
Private Sub btnSelectAllRecords_Click()
    Set dbs = CurrentDb
    SqlQuery = "UPDATE tblContacts SET tblContacts.IsSelected = 1"
    dbs.Execute SqlQuery, dbFailOnError
    Me.Refresh
End Sub
